...
int main(int argc, char **argv) {
  ...
  for (Int_t i = 0; i < nentries_wtn; i++) {
    ...
    vector<int> good_muon_idx, good_tau_idx;
    ...
    // Loop over muons in the event and apply loose selection so we
    // can still apply dimuon veto.
    for (auto imu = 0; imu < nMu; imu++) {
      // Loose Muon kinematic selection.
      if (muPt->at(imu) < 15 || fabs(muEta->at(imu)) > 2.4) {
        continue;
      }
      ...
      bool PassID = (muIDbit->at(imu) >> 0 & 1) == 1;
      if (!PassID) {
        continue;
      }
    ...
    if (good_muon_idx.size() > 1 && muCharge->at(good_muon_idx.at(0)) * muCharge->at(good_muon_idx.at(1)) < 0) {
      continue;
    } else if (good_muon_idx.size() == 0) {
      continue;
    }

    // Now apply tighter selection after we know there is only 1 good muon.
    auto idx = good_muon_idx.at(0);

    // Kinematic selection.
    if (muPt->at(idx) < 30 || fabs(muEta->at(idx)) > 2.1) {
      continue;
    }

    // Medium muon ID.
    bool PassID = (muIDbit->at(idx) >> 1 & 1) == 1;
    if (!PassID) {
      continue;
    }

    // Transverse mass cut to remove W events.
    float MuMetTranverseMass = TMass_F(muPt->at(idx), muPt->at(idx) * cos(muPhi->at(idx)), muPt->at(idx) * sin(muPhi->at(idx)), pfMET, pfMETPhi);
    ...
    // Set OS and SS.
    if (good_muon_idx.at(0) * good_tau_idx.at(0) < 0) {
      OS = true;
    } else {
      SS = true;
    }
